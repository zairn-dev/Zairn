...
describe('computeTrustScore input validation', () => {
  it('fails closed for malformed current or history points', () => {
    const malformedPoints: LocationPoint[] = [
      { ...current, lat: 91 },
      { ...current, lon: Number.POSITIVE_INFINITY },
      { ...current, accuracy: -1 },
      { ...current, timestamp: 'not-a-date' },
    ];

    for (const malformed of malformedPoints) {
      const result = computeTrustScore(malformed, normalHistory);
      expect(result.trustScore).toBe(0);
      expect(result.spoofingSuspected).toBe(true);
      expect(gateTrustScore(result)).toBe('deny');
    }

    expect(computeTrustScore(current, [
      { ...normalHistory[0], lon: -181 },
    ]).trustScore).toBe(0);
  });
});
...
describe('computeTrustScoreV2 default providers', () => {
  ...
  it('does not let self-consistent fixes erase a teleportation anomaly', () => {
    const distantHistory = [
      point(34.69, 135.5, 1),
      point(34.6901, 135.5001, 2),
    ];
    const v1 = computeTrustScore(current, distantHistory);
    const v2 = computeTrustScoreV2(current, distantHistory, {
      recentFixes: [
        fix(35.6797, 139.7597, 3),
        fix(35.6798, 139.7598, 2),
        fix(35.6799, 139.7599, 1),
        fix(35.6800, 139.7600, 0),
      ],
      networkHint: {
        lat: 34.69,
        lon: 135.5,
    ...
    expect(v2.signals.fixConsistency).toBeGreaterThanOrEqual(0.9);
    expect(v2.signals.networkConsistency).toBe(0.3);
    expect(v2.trustScore).toBeLessThanOrEqual(v1.trustScore);
    expect(gateTrustScore(v2)).not.toBe('proceed');
  });

  it('steps up on a network mismatch despite consistent GNSS fixes', () => {
    const result = computeTrustScoreV2(current, normalHistory, {
      recentFixes: [
        fix(35.6797, 139.7597, 3),
        fix(35.6798, 139.7598, 2),
  ...
  it('treats malformed network hints as contradictory evidence', () => {
    ...
    expect(v2.evidence).toEqual([{
      id: 'network-cross-check',
      weight: 2,
      score: 0,
      reason: 'invalid-network-hint',
    }]);
    expect(gateTrustScore(v2)).toBe('deny');
  });

  it('treats malformed RAIM fixes as contradictory evidence', () => {
    const result = computeTrustScoreV2(current, normalHistory, {
      recentFixes: [
        fix(35.68, 139.76, 2),
        fix(35.6801, 139.7601, 1),
        fix(35.6802, 181, 0),
      ],
    });
...
describe('TrustSignalProvider', () => {
  it('does not raise trust above the V1 plausibility ceiling', () => {
    const provider: TrustSignalProvider = {
      id: 'test-corroboration',
      weight: 10,
      evaluate: () => ({ score: 1, reason: 'attested' }),
    };
    const v1 = computeTrustScore(current, []);
    const v2 = computeTrustScoreV2(current, [], undefined, [provider]);
  ...
  it('protects scorer state with deeply frozen provider snapshots', () => {
    let receivedFrozenSnapshot = false;
    const provider: TrustSignalProvider = {
      id: 'mutation-attempt',
      weight: 1,
      evaluate: (input) => {
        receivedFrozenSnapshot =
          Object.isFrozen(input) &&
          Object.isFrozen(input.current) &&
          Object.isFrozen(input.history) &&
          Object.isFrozen(input.history[0]) &&
          Object.isFrozen(input.context) &&
          Object.isFrozen(input.baseResult) &&
          Object.isFrozen(input.baseResult.signals);

        try {
          (input.baseResult as { trustScore: number }).trustScore = 1;
        } catch {
          // Strict-mode assignment to the frozen snapshot is expected to fail.
        }
        try {
          (input.current as { lat: number }).lat = 0;
        } catch {
          // The caller-owned point is not exposed to providers.
        }
        return null;
      },
    };
    ...
    expect(receivedFrozenSnapshot).toBe(true);
    expect(result).toEqual(computeTrustScore(current, normalHistory));
    expect(current.lat).toBe(35.68);
  });
});

describe('gateTrustScore validation', () => {
  const result = computeTrustScore(current, normalHistory);

  it('rejects out-of-range and inverted thresholds', () => {
    expect(() => gateTrustScore(result, { proceed: -1 })).toThrow(RangeError);
    expect(() => gateTrustScore(result, { stepUp: 1.1 })).toThrow(RangeError);
    expect(() => gateTrustScore(result, {
      proceed: 0.2,
      stepUp: 0.8,
    })).toThrow('stepUp threshold must be less than or equal to proceed threshold');
  });

  it('rejects non-finite trust scores', () => {
    expect(() => gateTrustScore({
      ...result,
