...
export function createVerificationEngine(opts: VerificationEngineOptions): VerificationEngine {
  ...
  // Secret requirements are never persisted in plaintext (createDrop hashes
  // them into drop.proof_secret_hashes and strips params.secret — see
  // core.ts sanitizeProofConfig). Verification therefore requires the
  // hashed sidecar to be present, which in turn requires elevated
  // (service-role) drop access — anon/authenticated reads never see it, so
  // this verifier only functions for trusted server-side callers, matching
  // the unlock-drop Edge Function's server-side check.
  const verifySecret: ProofVerifier = async (req, sub, drop, requirementIndex) => {
    const submitted = (sub.data.secret as string) ?? '';
    const storedHash = requirementIndex != null
      ? drop.proof_secret_hashes?.[String(requirementIndex)]
      : undefined;
    const matched = storedHash ? await verifyPassword(submitted, storedHash) : false;
    return {
      method: 'secret',
      verified: matched,
  ...
  async function verify(
    ...
    for (const [idx, req] of config.requirements.entries()) {
      ...
      const result = await verifier(req, sub, drop, idx);
